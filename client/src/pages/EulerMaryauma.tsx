...
interface EulerMaryoumaFormData {
  ticker: Ticker;
  strike: number;
  r: number;
  sigma: number;
  T: number;
  type: OptionType;
  simulations: number; // Nombre de simulations (chemin)
  N: number; // Nombre de subdivisions
}

interface EulerMaryoumaResponse {
  ticker: string;
  spot_price: number;
  option_type: OptionType;
  simulations: number;
  simulation_matrix: number[][]; // Les trajectoires simulées
  option_price: number;
}

export default function EulerMaryouma() {
  const [form, setForm] = useState<EulerMaryoumaFormData>({
    ticker: "AAPL",
    strike: 100,
    r: 0.01,
    sigma: 0.2,
    ...
    type: "call",
    simulations: 10,
    N: 10, // Initial value for subdivisions (number of time steps)
  });

  const [result, setResult] = useState<
    EulerMaryoumaResponse | { error: string } | null
  >(null);
  ...
  const handleChange = (
    ...
    setForm((prev) => ({
      ...prev,
      [name]: ["strike", "r", "sigma", "T", "simulations", "N"].includes(name)
        ? parseFloat(value)
        : value,
    }));
  };
  ...
  return (
    <div style={{ maxWidth: "600px", margin: "auto" }}>
      ...
      >
        ...
        <div>
          <label htmlFor="N">Number of Subdivisions:</label>
          <input
            type="number"
            name="N"
            value={form.N}
